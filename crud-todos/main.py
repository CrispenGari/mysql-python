...
def insert():
    ...
    cursor.execute(INSERT_COMMAND.format(title))
    conn.commit()

def delete():
    try:
        id = int(input("Enter the todo id: ").strip())
        cursor.execute(DELETE_COMMAND.format(id))
        conn.commit()
    except:
...
def update():
    try:
        id = int(input("Enter the todo id: ").strip())
        cursor.execute(UPDATE_COMMAND.format(True, id))
        conn.commit()
    except:
        print(f"There's no todo of id: {id}")
...
def get(all=False):
    if all:
        ...
        id = int(input("Enter the todo id: ").strip())
        cursor.execute(SINGLE_TODO_COMMAND.format(id))
        try:
            rows = [(i, t, bool(c)) for (i, t, c) in cursor]
